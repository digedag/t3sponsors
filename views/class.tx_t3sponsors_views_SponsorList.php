...
class tx_t3sponsors_views_SponsorList extends tx_rnbase_view_Base
{
    ...
    public function createOutput($template, &$viewData, &$configurations, &$formatter)
    {
        ...
        if ($filter->hideResult()) {
            ...
            $items = array();
            $template = $filter->getMarker()->parseTemplate($template, $formatter, $this->getController()
                ->getConfId() . 'sponsor.filter.', 'SPONSOR');
        } else {
            $listBuilder = tx_rnbase::makeInstance('tx_rnbase_util_ListBuilder');
            $template = $listBuilder->render($items, $viewData, $template, 'tx_t3sponsors_marker_Sponsor', $this->getController()
                ->getConfId() . 'sponsor.', 'SPONSOR', $formatter);
        }

        if (tx_rnbase_util_BaseMarker::containsMarker($template, 'SPONSORMAP')) {
            $markerArray['###SPONSORMAP###'] = $this->getMap($items, $configurations, $this->getController()
                ->getConfId() . 'sponsor._map.', 'SPONSOR');
        }
